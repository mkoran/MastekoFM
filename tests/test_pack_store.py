...
def test_get_store_defaults_to_gcs():
    assert pack_store.get_store(None).kind == "gcs"
    assert pack_store.get_store("gcs").kind == "gcs"
    assert pack_store.get_store("drive_xlsx").kind == "drive_xlsx"
    # Unknown kind falls back to GCS rather than raising — keeps legacy docs safe.
    assert pack_store.get_store("future_kind").kind == "gcs"


def test_store_for_scenario_prefers_storage_kind_field():
    scn = {"storage_kind": "drive_xlsx", "drive_file_id": "abc"}
    assert pack_store.store_for_scenario(scn).kind == "drive_xlsx"
...
def test_store_for_scenario_infers_from_drive_file_id_when_kind_missing():
    ...
    assert pack_store.store_for_scenario({"drive_file_id": "abc"}).kind == "drive_xlsx"
    assert pack_store.store_for_scenario({"storage_path": "x.xlsx"}).kind == "gcs"


@patch("backend.app.services.pack_store.storage_service")
def test_gcs_store_read(mock_storage):
    mock_storage.download_xlsx.return_value = b"xlsx bytes"
    store = pack_store.GCSStore()
...
def test_drive_xlsx_store_write_creates_new_file(mock_drive):
    ...
    mock_drive.upload_file.assert_called_once_with(
        "folder-1", "scn.xlsx", b"xlsx", pack_store.XLSX_MIME, user_access_token="tok",
    )
...
def test_both_stores_return_none_open_url_when_no_artifact():
    assert pack_store.GCSStore().open_url({}) is None
    assert pack_store.DriveXlsxStore().open_url({}) is None
...
def test_drive_store_read_raises_on_download_failure(mock_drive):
    ...
    with pytest.raises(RuntimeError, match="Drive download failed"):
        pack_store.DriveXlsxStore().read_bytes({"drive_file_id": "x"}, user_access_token="t")


def test_mock_sanity():
    """Guard against test-mock drift: verify every public store has the required method set."""
    for store in (pack_store.GCSStore(), pack_store.DriveXlsxStore()):
...
def test_mock_call_count_on_gcs_write():
    store = pack_store.GCSStore()
    with patch("backend.app.services.pack_store.storage_service") as mock_storage:
...
def test_constants_match_expected_values():
    assert pack_store.STORAGE_KIND_GCS == "gcs"
    assert pack_store.STORAGE_KIND_DRIVE_XLSX == "drive_xlsx"
    assert pack_store.XLSX_MIME.startswith("application/vnd.openxmlformats")
